...
const databasePath = 'swordmasters/settings';
...
function firebaseConfigured() {
  return firebaseConfig && firebaseConfig.apiKey && !String(firebaseConfig.apiKey).includes('PASTE_');
}

function startFirebase() {
  if (!firebaseConfigured()) {
    setStatus('Firebase is not connected yet. Edit firebase-config.js with your Firebase config.');
    return;
  }
  try {
    ...
    onValue(ref(db, databasePath), snapshot => {
      const globalSettings = snapshot.val();
      if (globalSettings) {
        delete globalSettings.updatedAt;
        settings = { ...defaultSettings, ...globalSettings };
        applySettings(false);
        setStatus('Connected to Firebase. Changes are global for everyone.');
      } else {
        setStatus('Firebase connected. Save once in Admin to publish your first global settings.');
      }
    }, error => {
      connectedToFirebase = false;
      setStatus('Firebase read failed: ' + error.message);
    });
  } catch (error) {
    connectedToFirebase = false;
...
async function saveGlobalSettings(code) {
  if (code !== adminStaffCode) throw new Error('Wrong staff code.');
  if (!connectedToFirebase || !db) throw new Error('Firebase is not connected. Check firebase-config.js.');
  await set(ref(db, databasePath), { ...settings, updatedAt: serverTimestamp() });
}
...
function setAdminUnlocked(unlocked) {
  ...
  else { sessionStorage.removeItem(adminUnlockKey); lastStaffCode = ''; }
  updateAdminGate();
}
...
function tryUnlockAdmin() {
  ...
  if (code === adminStaffCode) {
    ...
    setStatus(connectedToFirebase ? 'Admin unlocked. Saves update everyone globally.' : 'Admin unlocked. Connect Firebase before saving globally.');
    return;
  }
...
if (adminForm) {
  adminForm.addEventListener('submit', async event => {
    ...
    try {
      ...
      setStatus('Saved globally. Everyone will see the update automatically.');
      fillAdminForm();
...
document.getElementById('copyShareLink')?.addEventListener('click', async () => {
  ...
  setStatus('Site link copied. This link reads live Firebase settings.');
});
...
console.log('Sword Masters loaded with Firebase global settings.');
