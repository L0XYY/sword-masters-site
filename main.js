...
if (adminForm) {
  adminForm.addEventListener('submit', async (event) => {
    ...
    if (!globalApiAvailable) {
      if (adminMessage) adminMessage.textContent = 'Not saved globally: this site is being served as static/client-only. Run it with npm start on a Node host.';
      return;
    }

    try {
      await saveGlobalSettings(lastStaffCode);
      if (adminMessage) adminMessage.textContent = 'Saved globally. Other devices update automatically within a few seconds.';
    } catch (error) {
...
if (copyShareLink) {
  copyShareLink.addEventListener('click', async () => {
    ...
    try {
      ...
      if (adminMessage) adminMessage.textContent = 'Publish link copied. Use this only for static hosting; Node hosting updates globally automatically.';
    } catch {
...
setInterval(loadGlobalSettings, 5000);
