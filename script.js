...
if (codeList) {
  ...
  if (codes.length) {
    if (codesIntro) codesIntro.textContent = "Redeem these active codes in game.";
    codeList.innerHTML = codes.map(code => `
      <article class="code-box">
...
const countdownDate = new Date(config.countdownDate || "2026-12-31T18:00").getTime();
...
function updateCountdown() {
  ...
  if (Number.isNaN(countdownDate)) {
    if (dateText) dateText.textContent = "Countdown date is not set correctly.";
    return;
  }
  ...
  if (remaining <= 0) {
    ...
    if (dateText) dateText.textContent = "Sword Masters is live!";
  ...
  if (dateText) {
    dateText.textContent = "Target launch: " + new Date(countdownDate).toLocaleString([], {
      year: "numeric",
      month: "long",
      day: "numeric",
      hour: "numeric",
